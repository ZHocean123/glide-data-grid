...
export function useColumnSort(p: Props): Result {
    ...
    const sortMap = computed(() => {
        const activeSorts = sorts.value
            .map((s, i) => ({ sort: s, col: sortCols.value[i] }))
    ...
    const getOriginalIndex = (index: number): number => {
        if (sortMap.value === undefined) return index;
        return sortMap.value[index];
    };

    const getCellContent = ([col, row]: [number, number]) => {
        if (sortMap.value === undefined) return getCellContentIn([col, row]);
        row = sortMap.value[row];
        return getCellContentIn([col, row]);
    };

    if (sortMap.value === undefined) {
