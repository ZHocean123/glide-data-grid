import type { DataGridSearchProps } from "../internal/data-grid/data-grid-types.js";
import type { CopyBuffer } from "../internal/data-grid/data-grid-types.js";
...
export function unquote(str: string): CopyBuffer {
    ...
    result.push(current);
    return result.map(r => r.map(c => ({ rawValue: c, formatted: c })));
}
