...
export function useKeybindingsWithDefaults(keybindingsIn?: Partial<Keybinds>): ComputedRef<RealizedKeybinds> {
    const keys = useDeepMemo(keybindingsIn);
    
    return computed(() => {
        if (keys.value === undefined) return realizeKeybinds(keybindingDefaults);
        const withBackCompatApplied = {
            ...keys.value,
            goToNextPage: keys.value?.goToNextPage ?? keys.value?.pageDown ?? keybindingDefaults.goToNextPage,
            goToPreviousPage: keys.value?.goToPreviousPage ?? keys.value?.pageUp ?? keybindingDefaults.goToPreviousPage,
            goToFirstCell: keys.value?.goToFirstCell ?? keys.value?.first ?? keybindingDefaults.goToFirstCell,
            goToLastCell: keys.value?.goToLastCell ?? keys.value?.last ?? keybindingDefaults.goToLastCell,
            selectToFirstCell: keys.value?.selectToFirstCell ?? keys.value?.first ?? keybindingDefaults.selectToFirstCell,
            selectToLastCell: keys.value?.selectToLastCell ?? keys.value?.last ?? keybindingDefaults.selectToLastCell,
        };
