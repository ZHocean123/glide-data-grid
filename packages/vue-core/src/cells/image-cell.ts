import { h } from "vue";
import ImageOverlayEditor from "../internal/data-grid-overlay-editor/private/ImageOverlayEditor.vue";
...
// Vue component for image cell overlay editor
const ImageOverlayEditorComponent = {
    name: "ImageOverlayEditorComponent",
    props: {
        urls: {
            type: Array,
            default: () => []
...
export const imageCellRenderer: InternalCellRenderer<ImageCell> = {
    ...
    provideEditor: () => {
        return {
            editor: (props: any) => {
                const { value, onFinishedEditing, imageEditorOverride } = props;
                const ImageEditor = imageEditorOverride ?? ImageOverlayEditorComponent;
