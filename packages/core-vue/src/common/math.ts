...
import type { FillHandleDirection, Rectangle } from '../types/data-grid-types.js';
...
// Helper function for getClosestRect
function itemIsInRect(item: [number, number], rect: Rectangle): boolean {
    return item[0] >= rect.x && item[0] < rect.x + rect.width && item[1] >= rect.y && item[1] < rect.y + rect.height;
}