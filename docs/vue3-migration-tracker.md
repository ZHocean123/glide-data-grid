...
- **Latest Update:** `gridSelectionHasItem`, `cellIsSelected`, and related helpers now live in `src/shared/selection.ts`; React exports re-export the shared implementations.
- **Risks/Blockers:** Verify downstream modules transition smoothly as more helpers migrate; watch for circular deps.
- **Next Actions:** Continue migrating geometry helpers (e.g., `getStickyWidth`, `getEffectiveColumns`) and add shared tests if coverage is thin.
