...
- **Latest Update:** Selection and column helpers plus geometry utilities (`getColumnIndexForX`, `getRowIndexForY`) now live in `src/shared/*` with direct unit coverage.
- **Risks/Blockers:** Monitor for duplicated exports when removing helpers; ensure consumer imports updated as more logic moves.
- **Next Actions:** Continue migrating remaining compute helpers (e.g., `computeBounds`) and start drafting Vue adapter wrappers.
