...
- **Latest Update:** Selection helpers plus column-derived utilities (`remapForDnDState`, `getStickyWidth`, `getEffectiveColumns`) live in `src/shared`; React exports re-export them.
- **Risks/Blockers:** Keep watch for circular imports; ensure compute helpers remain covered by tests as they migrate.
- **Next Actions:** Add shared unit tests for selection/column utils and continue migrating remaining pure logic (e.g., `getRowIndexForY`).
